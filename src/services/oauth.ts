...
// Interface for OAuth request tokens
interface OAuthData {
  requestToken: string;
...
async function getRequestToken(req: IncomingMessage, res: ServerResponse): Promise<void> {
...
async function verifyRequestToken(
  ...
  const requestToken: string | null = searchParams.get('oauth_token');
  const oauthVerifier: string | null = searchParams.get('oauth_verifier');
  ...
  const requestTokenSecret: string | undefined = db.oauth.get(requestToken);
...
function getcert(filename: string): Buffer {
  try {
    return readFileSync(resolve('.', filename));
  } catch (err: any) {
...
const server = createServer(options, function (req: IncomingMessage, res: ServerResponse) {
