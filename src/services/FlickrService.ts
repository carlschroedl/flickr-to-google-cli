...
export class FlickrService {
  ...
  async getAlbums(): Promise<FlickrAlbum[]> {
    try {
      ...
      const albums: FlickrAlbum[] = albumsData.albums.map(
        async (albumData: any) => await this.mapToFlickrAlbum(albumData)
      );
   ...
   */
  async mapToFlickrAlbum(albumData: any): Promise<FlickrAlbum> {
    ...
    for (const photoId of albumData.photos) {
      try {
        ...
        Logger.warning(`Failed to load photo ${photoId} for album ${albumData.albumId}`, error);
      }
    }

    const mappedAlbum: FlickrAlbum = {
      id: albumData.id,
